...
class StataWriter(object):
    ...
    def __init__(self, fname, data, convert_dates=None, encoding="latin-1",
        ...
        if input_types.is_using_pandas(data, None):
            self._prepare_pandas(data)

        elif input_types.is_array_like(data, None):
            data = np.asarray(data)
            if input_types.is_structured_ndarray(data):
    ...
    def _write(self, to_write):
        ...
        if not isinstance(to_write, bytes):
            to_write = to_write.encode('latin-1')  # TODO: utf-8?
    ...
    def _write_descriptors(self, typlist=None, varlist=None, srtlist=None,
        ...
        for name in self.varlist:
            ...
            name32 = name[:32]
            if not isinstance(name32, unicode):
                name32 = name32.decode('latin-1')
            name = _pad_bytes(name32, 33)
            self._write(name)
    ...
    def _write_data_dates(self):
        ...
        for row in data:
            ...
            for i,var in enumerate(row):
                ...
                if typ <= 244:  # we've got a string
                    if pd.isnull(var):  # TODO: transition to isna?
                        var = ""  # missing string
                    if len(var) < typ:
                        var = _pad_bytes(var, len(var) + 1)
                    self._write(var)
