...
class ModelData(object):
    ...
    def _get_yarr(self, endog):
        if input_types.is_structured_ndarray(endog):
            endog = input_types.struct_to_ndarray(endog)
    ...
    def _get_xarr(self, exog):
        if input_types.is_structured_ndarray(exog):
            exog = input_types.struct_to_ndarray(exog)
...
def handle_data_class_factory(endog, exog):
    ...
    Given inputs
    """
    if input_types.is_using_ndarray_type(endog, exog):
        klass = ModelData
    elif input_types.is_using_pandas(endog, exog):
        klass = PandasData
    elif input_types.is_using_patsy(endog, exog):
        klass = PatsyData
    # keep this check last
    elif input_types.is_using_ndarray(endog, exog):
        klass = ModelData
    else:
