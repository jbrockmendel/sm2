...
#
#    def test_dates(self):
#        assert_equal(data.wrap_output(self.dates_input, 'dates'),
#                                self.dates_result)
...
class TestArrays(object):
    ...
    def test_orig(self):
        assert_equal(self.data.orig_endog, self.endog)
        assert_equal(self.data.orig_exog, self.exog)

    def test_endogexog(self):
        assert_equal(self.data.endog, self.endog)
        assert_equal(self.data.exog, self.exog)

    def test_attach(self):
        data = self.data
        # this makes sure what the wrappers need work but not the wrapped
        # results themselves
        assert_equal(data.wrap_output(self.col_input, 'columns'),
                                self.col_result)
        assert_equal(data.wrap_output(self.row_input, 'rows'),
                                self.row_result)
        assert_equal(data.wrap_output(self.cov_input, 'cov'),
                                self.cov_result)

    def test_names(self):
        data = self.data
        assert_equal(data.xnames, self.xnames)
        assert_equal(data.ynames, self.ynames)

    def test_labels(self):
        # HACK: because numpy master after NA stuff assert_equal fails on
        # pandas indices
        assert_(np.all(self.data.row_labels == self.row_labels))
...
class TestArrays2dEndog(TestArrays):
    ...
    def test_endogexog(self):
        assert_equal(self.data.endog, self.endog.squeeze())
        assert_equal(self.data.exog, self.exog)
...
class TestArrays1dExog(TestArrays):
    ...
    def test_orig(self):
        assert_equal(self.data.orig_endog, self.endog)
        assert_equal(self.data.orig_exog, self.exog.squeeze())
...
class TestDataFrames(TestArrays):
    ...
    def test_endogexog(self):
        assert_equal(self.data.endog, self.endog.values.squeeze())
        assert_equal(self.data.exog, self.exog.values)

    def test_attach(self):
        data = self.data
        # this makes sure what the wrappers need work but not the wrapped
        # results themselves
        tm.assert_series_equal(data.wrap_output(self.col_input,
                                                  'columns'),
                                self.col_result)
...
class TestRecarrays(TestArrays):
    ...
    def test_endogexog(self):
        assert_equal(self.data.endog, self.endog.view(float, type=np.ndarray))
        assert_equal(self.data.exog, self.exog.view((float, 3), type=np.ndarray))
...
class TestSeriesSeries(TestDataFrames):
    ...
    def test_endogexog(self):
        assert_equal(self.data.endog, self.endog.values.squeeze())
        assert_equal(self.data.exog, self.exog.values[:,None])
...
class TestMultipleEqsArrays(TestArrays):
    ...
    def test_attach(self):
        ...
        assert_equal(data.wrap_output(self.cov_input, 'cov'),
                                self.cov_result)
        assert_equal(data.wrap_output(self.cov_eq_input, 'cov_eq'),
                                self.cov_eq_result)
        assert_equal(data.wrap_output(self.col_eq_input,
                                                 'columns_eq'),
                                self.col_eq_result)
...
class TestMultipleEqsDataFrames(TestDataFrames):
    ...
    def test_attach(self):
        ...
        tm.assert_frame_equal(data.wrap_output(self.cov_input, 'cov'),
                                self.cov_result)
        tm.assert_frame_equal(data.wrap_output(self.cov_eq_input,
                                    'cov_eq'),
                                self.cov_eq_result)
...
class TestMissingArray(object):
    ...
    def test_raise(self):
        assert_raises(Exception, sm_data.handle_data,
                                            (self.y, self.X, 'raise'))
    ...
    def test_drop(self):
        ...
        assert_array_equal(data.endog, y)
        assert_array_equal(data.exog, X)

    def test_none(self):
        data = sm_data.handle_data(self.y, self.X, 'none', hasconst=False)
        assert_array_equal(data.endog, self.y)
        assert_array_equal(data.exog, self.X)
    ...
    def test_extra_kwargs_2d(self):
        ...
        sigma = sigma[idx][:,idx]
        assert_array_equal(data.sigma, sigma)
    ...
    def test_extra_kwargs_1d(self):
        ...
        idx = ~np.isnan(np.c_[self.y, self.X]).any(axis=1)
        weights = weights[idx]
        assert_array_equal(data.weights, weights)
...
class TestMissingPandas(object):
    ...
    def test_drop(self):
        ...
        X = X.loc[idx]
        data = sm_data.handle_data(self.y, self.X, 'drop')
        assert_array_equal(data.endog, y.values)
        tm.assert_series_equal(data.orig_endog, self.y.loc[idx])
        assert_array_equal(data.exog, X.values)
        tm.assert_frame_equal(data.orig_exog, self.X.loc[idx])

    def test_none(self):
        data = sm_data.handle_data(self.y, self.X, 'none', hasconst=False)
        assert_array_equal(data.endog, self.y.values)
        assert_array_equal(data.exog, self.X.values)
    ...
    def test_labels(self):
        ...
        data = sm_data.handle_data(self.y, self.X, 'drop')
        assert_(data.row_labels.equals(labels))
...
class TestConstant(object):
    ...
    def test_array_constant(self):
        ...
        assert_equal(data.k_constant, 1)
        assert_equal(data.const_idx, 6)
    ...
    def test_pandas_noconstant(self):
        exog = self.data.exog.copy()
        data = sm_data.handle_data(self.data.endog, exog)
        assert_equal(data.k_constant, 0)
        assert_equal(data.const_idx, None)
...
class TestHandleMissing(object):
    ...
    def test_arrays(self):
        ...
        assert_array_equal(data['endog'], y_exp)
        assert_array_equal(data['exog'], X_exp)
...
# TODO: enable once OLS is ported
'''
def test_alignment():
    # See GH#206
    ...
    assert_raises(ValueError, OLS, *(endog, exog))
'''
...
def test_dtype_object():
    ...
    assert_raises(ValueError, sm_data.handle_data, y, df)
