...
class TestMixedLM(object):
    ...
    @pytest.mark.slow
    @pytest.mark.parametrize('use_sqrt', [
        False,
        pytest.param(True,
                     marks=pytest.mark.xfail(reason="to_packed and from_packed "
                                                    "dont round-trip"
                                                    "correctly"))])
    ...
    def test_vcomp_3(self):
        ...
        model = MixedLM.from_formula("y ~ 1", groups="groups",
                                     ...
                                     data=df)
        result = model.fit()
        # result.summary()  # summary tests disabled b/c it is not ported
    ...
    def test_formulas(self):
        ...
        # test the names
        assert mod1.data.xnames == ["x1", "x2", "x3", "x4"]
        assert mod1.data.exog_re_names == ["x_re1"]
        assert mod1.data.exog_re_names_full == ["x_re1 Var"]
        rslt1 = mod1.fit()
        ...
        assert mod2.data.xnames == ["exog0", "exog1", "exog2", "exog3"]
        assert mod2.data.exog_re_names == ["exog_re"]
        assert mod2.data.exog_re_names_full == ["exog_re Var"]
        ...
        mod3 = MixedLM.from_formula(fml, df, re_formula=re_fml,
                                    groups="groups")
        assert mod3.data.xnames == ["exog0", "exog1", "exog2", "exog3"]
        assert mod3.data.exog_re_names == ["exog_re"]
        assert mod3.data.exog_re_names_full == ["exog_re Var"]
        ...
        assert mod5.data.exog_re_names == ["groups"]
        assert mod5.data.exog_re_names_full == ["groups Var"]
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            rslt5 = mod5.fit()
...
# Copied from bashtage's GH#3847
...
def test_mixed_lm_wrapper():
    ...
    assert mod2.data.xnames == xnames
    assert mod2.data.exog_re_names == re_names
    assert mod2.data.exog_re_names_full == re_names_full

    params = result.params
    assert params.index.tolist() == xnames + re_names_full
    bse = result.bse
    assert bse.index.tolist() == xnames + re_names_full
    tvalues = result.tvalues
    assert tvalues.index.tolist() == xnames + re_names_full
    cov_params = result.cov_params()
    assert cov_params.index.tolist() == xnames + re_names_full
    assert cov_params.columns.tolist() == xnames + re_names_full
    fe = result.fe_params
    assert fe.index.tolist() == xnames
    bse_fe = result.bse_fe
    assert bse_fe.index.tolist() == xnames
    cov_re = result.cov_re
    assert cov_re.index.tolist() == re_names
    assert cov_re.columns.tolist() == re_names
    cov_re_u = result.cov_re_unscaled
    assert cov_re_u.index.tolist() == re_names
    assert cov_re_u.columns.tolist() == re_names
    bse_re = result.bse_re
    assert bse_re.index.tolist() == re_names_full
...
def test_random_effects():
    ...
    assert len(re) == ngrp
    assert isinstance(re[0], pd.Series)
    assert len(re[0]) == 1
...
@pytest.mark.parametrize('include_re', [False, True])
@pytest.mark.parametrize('include_vc', [False, True])
def test_handle_missing(include_re, include_vc):
    ...
    with warnings.catch_warnings():
        ...
        assert len(result1.fittedvalues) == result1.nobs
