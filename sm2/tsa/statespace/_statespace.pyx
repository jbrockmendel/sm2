...
from sm2.src.math cimport zlog, zabs, NPY_PI
...
from sm2.src.blas_lapack cimport *
...
cdef sswap_t *sswap = <sswap_t*>Capsule_AsVoidPtr(blas.sswap._cpointer)
...
cdef dswap_t *dswap = <dswap_t*>Capsule_AsVoidPtr(blas.dswap._cpointer)
...
cdef cswap_t *cswap = <cswap_t*>Capsule_AsVoidPtr(blas.cswap._cpointer)
...
cdef zswap_t *zswap = <zswap_t*>Capsule_AsVoidPtr(blas.zswap._cpointer)
    ...
    *See Durbin and Koopman (2012), Chapter 4 for all notation*
    ...
    def initialize_stationary(self, complex_step=False):
        ...
        if np.sum(self.state_intercept) == 0:
            ...
            eye = np.eye(self.k_states).astype(np.float32)
            T = np.array(self.transition[:,:,0], dtype=np.float32)
            # (I - T)^{-1} c = x => (I - T) x = c
            self.initial_state = np.linalg.solve(
                eye - T, np.array(self.state_intercept[:, 0], dtype=np.float32))
...
cdef int sselect_state_cov(int k_states, int k_posdef,
                                    ...
                                    cnp.float32_t * state_cov,
                                    cnp.float32_t * selected_state_cov):
    cdef:
...
cdef cnp.float32_t sinverse_missing_conventional(sKalmanFilter kfilter, cnp.float32_t determinant)  except *:
...
cdef cnp.float32_t sloglikelihood_missing_conventional(sKalmanFilter kfilter, cnp.float32_t determinant):
    return 0.0
...
cdef cnp.float32_t sloglikelihood_conventional(sKalmanFilter kfilter, cnp.float32_t determinant):
    # Constants
    cdef:
...
cdef cnp.float32_t sinverse_univariate(sKalmanFilter kfilter, cnp.float32_t determinant) except *:
    """
...
cdef cnp.float32_t sfactorize_cholesky(sKalmanFilter kfilter, cnp.float32_t determinant) except *:
    """
...
cdef cnp.float32_t sfactorize_lu(sKalmanFilter kfilter, cnp.float32_t determinant) except *:
    """
...
cdef cnp.float32_t sinverse_cholesky(sKalmanFilter kfilter, cnp.float32_t determinant) except *:
    """
...
cdef cnp.float32_t sinverse_lu(sKalmanFilter kfilter, cnp.float32_t determinant) except *:
    """
    inverse_cholesky(self, determinant)
...
cdef cnp.float32_t ssolve_cholesky(sKalmanFilter kfilter, cnp.float32_t determinant) except *:
    """
...
cdef cnp.float32_t ssolve_lu(sKalmanFilter kfilter, cnp.float32_t determinant) except *:
    """
    inverse_cholesky(self, determinant)
...
cdef class sKalmanFilter(object):
    ...
    cdef readonly cnp.float32_t [::1,:] tmp0, tmp1, tmp3
    cdef readonly cnp.float32_t [:] tmp2
    ...
    def __init__(self,
        ...
        self.tmp1 = cnp.PyArray_ZEROS(2, dim2, cnp.NPY_FLOAT32, FORTRAN)
        self._tmp1 = &self.tmp1[0, 0]

        # Holds arrays of dimension $(p \times 1)$
        dim1[0] = self.k_endog;
        self.tmp2 = cnp.PyArray_ZEROS(1, dim1, cnp.NPY_FLOAT32, FORTRAN)
        self._tmp2 = &self.tmp2[0]

        # Holds arrays of dimension $(p \times m)$
        dim2[0] = self.k_endog; dim2[1] = self.k_states;
        self.tmp3 = cnp.PyArray_ZEROS(2, dim2, cnp.NPY_FLOAT32, FORTRAN)
...
cpdef _scompute_coefficients_from_multivariate_pacf(cnp.float32_t [::1,:] partial_autocorrelations,
...
cpdef _sconstrain_sv_less_than_one(cnp.float32_t [::1,:] unconstrained, int order, int k_endog):
    """
...
cdef int _scopy_missing_submatrix(cnp.float32_t * a, cnp.float32_t * b, int * missing, int n):
    ...
    # Perform replacement
    for i in range(nobs):
        scopy(&nobs, &a[i*n], &inc, &b[i*n], &inc)
...
cdef int _scopy_missing_cols(cnp.float32_t * a, cnp.float32_t * b, int * missing, int n, int m):
    ...
    # Perform replacement
    for i in range(nobs):
        scopy(&n, &a[i*n], &inc, &b[i*n], &inc)


cpdef int scopy_missing_matrix(cnp.float32_t [::1, :, :] A, cnp.float32_t [::1, :, :] B, int [::1, :] missing, int missing_rows, int missing_cols, int diagonal) except *:
...
cpdef int scopy_index_matrix(cnp.float32_t [::1, :, :] A, cnp.float32_t [::1, :, :] B, int [::1, :] index, int index_rows, int index_cols, int diagonal) except *:
    cdef int n, m, T, t, A_T, A_t = 0, time_varying
    ...
    *See Durbin and Koopman (2012), Chapter 4 for all notation*
    ...
    def initialize_stationary(self, complex_step=False):
        ...
        if np.sum(self.state_intercept) == 0:
            ...
            eye = np.eye(self.k_states).astype(float)
            T = np.array(self.transition[:,:,0], dtype=float)
            # (I - T)^{-1} c = x => (I - T) x = c
            self.initial_state = np.linalg.solve(
                eye - T, np.array(self.state_intercept[:, 0], dtype=float))
...
cdef int dselect_state_cov(int k_states, int k_posdef,
                                    ...
                                    cnp.float64_t * state_cov,
                                    cnp.float64_t * selected_state_cov):
    cdef:
...
cdef cnp.float64_t dinverse_missing_conventional(dKalmanFilter kfilter, cnp.float64_t determinant)  except *:
    # Since the inverse of the forecast error covariance matrix is not
    # stored, we don't need to fill it (e.g. with NPY_NAN values). Instead,
    # just do a noop here and return a zero determinant ($|0|$).
    return 0.0

cdef cnp.float64_t dloglikelihood_missing_conventional(dKalmanFilter kfilter, cnp.float64_t determinant):
    return 0.0
...
cdef cnp.float64_t dloglikelihood_conventional(dKalmanFilter kfilter, cnp.float64_t determinant):
    # Constants
    cdef:
...
cdef cnp.float64_t dinverse_univariate(dKalmanFilter kfilter, cnp.float64_t determinant) except *:
    """
    Factorize the forecast error covariance matrix using simple division
    in the case that the observations are univariate.
...
cdef cnp.float64_t dfactorize_cholesky(dKalmanFilter kfilter, cnp.float64_t determinant) except *:
    """
    Factorize the forecast error covariance matrix using a Cholesky
    decomposition. Called by either of the `solve_cholesky` or
    `invert_cholesky` routines.
...
cdef cnp.float64_t dfactorize_lu(dKalmanFilter kfilter, cnp.float64_t determinant) except *:
    """
    Factorize the forecast error covariance matrix using an LU
    decomposition. Called by either of the `solve_lu` or `invert_lu`
    routines.
...
cdef cnp.float64_t dinverse_cholesky(dKalmanFilter kfilter, cnp.float64_t determinant) except *:
    """
    inverse_cholesky(self, determinant)
...
cdef cnp.float64_t dinverse_lu(dKalmanFilter kfilter, cnp.float64_t determinant) except *:
    """
    inverse_cholesky(self, determinant)
...
cdef cnp.float64_t dsolve_cholesky(dKalmanFilter kfilter, cnp.float64_t determinant) except *:
    """
    solve_cholesky(self, determinant)
...
cdef cnp.float64_t dsolve_lu(dKalmanFilter kfilter, cnp.float64_t determinant) except *:
    """
    inverse_cholesky(self, determinant)
...
cdef class dKalmanFilter(object):
    ...
    # These hold the memory allocations of the unnamed temporary arrays
    cdef readonly cnp.float64_t [::1,:] tmp0, tmp1, tmp3
    cdef readonly cnp.float64_t [:] tmp2
    ...
    def __init__(self,
        ...
        # Holds arrays of dimension $(m \times p)$
        dim2[0] = self.k_states; dim2[1] = self.k_endog;
        self.tmp1 = cnp.PyArray_ZEROS(2, dim2, cnp.NPY_FLOAT64, FORTRAN)
        self._tmp1 = &self.tmp1[0, 0]

        # Holds arrays of dimension $(p \times 1)$
        dim1[0] = self.k_endog;
        self.tmp2 = cnp.PyArray_ZEROS(1, dim1, cnp.NPY_FLOAT64, FORTRAN)
        self._tmp2 = &self.tmp2[0]

        # Holds arrays of dimension $(p \times m)$
        dim2[0] = self.k_endog; dim2[1] = self.k_states;
        self.tmp3 = cnp.PyArray_ZEROS(2, dim2, cnp.NPY_FLOAT64, FORTRAN)
        self._tmp3 = &self.tmp3[0, 0]
...
cpdef _dcompute_coefficients_from_multivariate_pacf(cnp.float64_t [::1,:] partial_autocorrelations,
...
cpdef _dconstrain_sv_less_than_one(cnp.float64_t [::1,:] unconstrained, int order, int k_endog):
    """
    Transform arbitrary matrices to matrices with singular values less than
    one.
...
cdef int _dcopy_missing_submatrix(cnp.float64_t * a, cnp.float64_t * b, int * missing, int n):
    ...
    # Perform replacement
    for i in range(nobs):
        dcopy(&nobs, &a[i*n], &inc, &b[i*n], &inc)
...
cdef int _dcopy_missing_cols(cnp.float64_t * a, cnp.float64_t * b, int * missing, int n, int m):
    ...
    # Perform replacement
    for i in range(nobs):
        dcopy(&n, &a[i*n], &inc, &b[i*n], &inc)


cpdef int dcopy_missing_matrix(cnp.float64_t [::1, :, :] A, cnp.float64_t [::1, :, :] B, int [::1, :] missing, int missing_rows, int missing_cols, int diagonal) except *:
    cdef int n, m, T, t, A_T, A_t = 0, time_varying
...
cpdef int dcopy_index_matrix(cnp.float64_t [::1, :, :] A, cnp.float64_t [::1, :, :] B, int [::1, :] index, int index_rows, int index_cols, int diagonal) except *:
    cdef int n, m, T, t, A_T, A_t = 0, time_varying
    ...
    *See Durbin and Koopman (2012), Chapter 4 for all notation*
    ...
    def initialize_stationary(self, complex_step=False):
        ...
        if np.sum(self.state_intercept) == 0:
            ...
            eye = np.eye(self.k_states).astype(np.complex64)
            T = np.array(self.transition[:,:,0], dtype=np.complex64)
            # (I - T)^{-1} c = x => (I - T) x = c
            self.initial_state = np.linalg.solve(
                eye - T, np.array(self.state_intercept[:, 0], dtype=np.complex64))
...
cdef int cselect_state_cov(int k_states, int k_posdef,
                                    ...
                                    cnp.complex64_t * state_cov,
                                    cnp.complex64_t * selected_state_cov):
    cdef:
...
cdef cnp.complex64_t cinverse_missing_conventional(cKalmanFilter kfilter, cnp.complex64_t determinant)  except *:
    # Since the inverse of the forecast error covariance matrix is not
    # stored, we don't need to fill it (e.g. with NPY_NAN values). Instead,
    # just do a noop here and return a zero determinant ($|0|$).
    return 0.0

cdef cnp.complex64_t cloglikelihood_missing_conventional(cKalmanFilter kfilter, cnp.complex64_t determinant):
    return 0.0
...
cdef cnp.complex64_t cloglikelihood_conventional(cKalmanFilter kfilter, cnp.complex64_t determinant):
    # Constants
    cdef:
        cnp.complex64_t loglikelihood
        int inc = 1
        cnp.complex64_t alpha = 1.0
        cnp.complex64_t beta = 0.0
...
cdef cnp.complex64_t cinverse_univariate(cKalmanFilter kfilter, cnp.complex64_t determinant) except *:
    """
    Factorize the forecast error covariance matrix using simple division
    in the case that the observations are univariate.
...
cdef cnp.complex64_t cfactorize_cholesky(cKalmanFilter kfilter, cnp.complex64_t determinant) except *:
    """
    Factorize the forecast error covariance matrix using a Cholesky
    decomposition. Called by either of the `solve_cholesky` or
    `invert_cholesky` routines.
...
cdef cnp.complex64_t cfactorize_lu(cKalmanFilter kfilter, cnp.complex64_t determinant) except *:
    """
    Factorize the forecast error covariance matrix using an LU
    decomposition. Called by either of the `solve_lu` or `invert_lu`
    routines.
...
cdef cnp.complex64_t cinverse_cholesky(cKalmanFilter kfilter, cnp.complex64_t determinant) except *:
    """
    inverse_cholesky(self, determinant)
...
cdef cnp.complex64_t cinverse_lu(cKalmanFilter kfilter, cnp.complex64_t determinant) except *:
    """
    inverse_cholesky(self, determinant)
...
cdef cnp.complex64_t csolve_cholesky(cKalmanFilter kfilter, cnp.complex64_t determinant) except *:
    """
    solve_cholesky(self, determinant)
...
cdef cnp.complex64_t csolve_lu(cKalmanFilter kfilter, cnp.complex64_t determinant) except *:
    """
    inverse_cholesky(self, determinant)
...
cdef class cKalmanFilter(object):
    ...
    # These hold the memory allocations of the unnamed temporary arrays
    cdef readonly cnp.complex64_t [::1,:] tmp0, tmp1, tmp3
    cdef readonly cnp.complex64_t [:] tmp2
    ...
    def __init__(self,
        ...
        # Holds arrays of dimension $(m \times p)$
        dim2[0] = self.k_states; dim2[1] = self.k_endog;
        self.tmp1 = cnp.PyArray_ZEROS(2, dim2, cnp.NPY_COMPLEX64, FORTRAN)
        self._tmp1 = &self.tmp1[0, 0]

        # Holds arrays of dimension $(p \times 1)$
        dim1[0] = self.k_endog;
        self.tmp2 = cnp.PyArray_ZEROS(1, dim1, cnp.NPY_COMPLEX64, FORTRAN)
        self._tmp2 = &self.tmp2[0]

        # Holds arrays of dimension $(p \times m)$
        dim2[0] = self.k_endog; dim2[1] = self.k_states;
        self.tmp3 = cnp.PyArray_ZEROS(2, dim2, cnp.NPY_COMPLEX64, FORTRAN)
        self._tmp3 = &self.tmp3[0, 0]
...
cpdef _ccompute_coefficients_from_multivariate_pacf(cnp.complex64_t [::1,:] partial_autocorrelations,
...
cpdef _cconstrain_sv_less_than_one(cnp.complex64_t [::1,:] unconstrained, int order, int k_endog):
    """
    Transform arbitrary matrices to matrices with singular values less than
    one.
...
cdef int _ccopy_missing_submatrix(cnp.complex64_t * a, cnp.complex64_t * b, int * missing, int n):
    ...
    # Perform replacement
    for i in range(nobs):
        ccopy(&nobs, &a[i*n], &inc, &b[i*n], &inc)
...
cdef int _ccopy_missing_cols(cnp.complex64_t * a, cnp.complex64_t * b, int * missing, int n, int m):
    ...
    # Perform replacement
    for i in range(nobs):
        ccopy(&n, &a[i*n], &inc, &b[i*n], &inc)


cpdef int ccopy_missing_matrix(cnp.complex64_t [::1, :, :] A, cnp.complex64_t [::1, :, :] B, int [::1, :] missing, int missing_rows, int missing_cols, int diagonal) except *:
    cdef int n, m, T, t, A_T, A_t = 0, time_varying
...
cpdef int ccopy_index_matrix(cnp.complex64_t [::1, :, :] A, cnp.complex64_t [::1, :, :] B, int [::1, :] index, int index_rows, int index_cols, int diagonal) except *:
    cdef int n, m, T, t, A_T, A_t = 0, time_varying
    ...
    *See Durbin and Koopman (2012), Chapter 4 for all notation*
    ...
    def initialize_stationary(self, complex_step=False):
        ...
        if np.sum(self.state_intercept) == 0:
            ...
            eye = np.eye(self.k_states).astype(complex)
            T = np.array(self.transition[:,:,0], dtype=complex)
            # (I - T)^{-1} c = x => (I - T) x = c
            self.initial_state = np.linalg.solve(
                eye - T, np.array(self.state_intercept[:, 0], dtype=complex))
...
cdef int zselect_state_cov(int k_states, int k_posdef,
                                    ...
                                    cnp.complex128_t * state_cov,
                                    cnp.complex128_t * selected_state_cov):
    cdef:
...
cdef cnp.complex128_t zinverse_missing_conventional(zKalmanFilter kfilter, cnp.complex128_t determinant)  except *:
    # Since the inverse of the forecast error covariance matrix is not
    # stored, we don't need to fill it (e.g. with NPY_NAN values). Instead,
    # just do a noop here and return a zero determinant ($|0|$).
    return 0.0

cdef cnp.complex128_t zloglikelihood_missing_conventional(zKalmanFilter kfilter, cnp.complex128_t determinant):
    return 0.0
...
cdef cnp.complex128_t zloglikelihood_conventional(zKalmanFilter kfilter, cnp.complex128_t determinant):
    # Constants
    cdef:
        cnp.complex128_t loglikelihood
        int inc = 1
        cnp.complex128_t alpha = 1.0
        cnp.complex128_t beta = 0.0
...
cdef cnp.complex128_t zinverse_univariate(zKalmanFilter kfilter, cnp.complex128_t determinant) except *:
    """
    Factorize the forecast error covariance matrix using simple division
    in the case that the observations are univariate.
...
cdef cnp.complex128_t zfactorize_cholesky(zKalmanFilter kfilter, cnp.complex128_t determinant) except *:
    """
    Factorize the forecast error covariance matrix using a Cholesky
    decomposition. Called by either of the `solve_cholesky` or
    `invert_cholesky` routines.
...
cdef cnp.complex128_t zfactorize_lu(zKalmanFilter kfilter, cnp.complex128_t determinant) except *:
    """
    Factorize the forecast error covariance matrix using an LU
    decomposition. Called by either of the `solve_lu` or `invert_lu`
    routines.
...
cdef cnp.complex128_t zinverse_cholesky(zKalmanFilter kfilter, cnp.complex128_t determinant) except *:
    """
    inverse_cholesky(self, determinant)
...
cdef cnp.complex128_t zinverse_lu(zKalmanFilter kfilter, cnp.complex128_t determinant) except *:
    """
    inverse_cholesky(self, determinant)
...
cdef cnp.complex128_t zsolve_cholesky(zKalmanFilter kfilter, cnp.complex128_t determinant) except *:
    """
    solve_cholesky(self, determinant)
...
cdef cnp.complex128_t zsolve_lu(zKalmanFilter kfilter, cnp.complex128_t determinant) except *:
    """
    inverse_cholesky(self, determinant)
...
cdef class zKalmanFilter(object):
    ...
    # These hold the memory allocations of the unnamed temporary arrays
    cdef readonly cnp.complex128_t [::1,:] tmp0, tmp1, tmp3
    cdef readonly cnp.complex128_t [:] tmp2
    ...
    cdef cnp.complex128_t (*inversion)(
        zKalmanFilter, cnp.complex128_t
    ) except *
    cdef int (*updating)(
        zKalmanFilter
    )
    ...
    def __init__(self,
        ...
        # Holds arrays of dimension $(m \times p)$
        dim2[0] = self.k_states; dim2[1] = self.k_endog;
        self.tmp1 = cnp.PyArray_ZEROS(2, dim2, cnp.NPY_COMPLEX128, FORTRAN)
        self._tmp1 = &self.tmp1[0, 0]

        # Holds arrays of dimension $(p \times 1)$
        dim1[0] = self.k_endog;
        self.tmp2 = cnp.PyArray_ZEROS(1, dim1, cnp.NPY_COMPLEX128, FORTRAN)
        self._tmp2 = &self.tmp2[0]

        # Holds arrays of dimension $(p \times m)$
        dim2[0] = self.k_endog; dim2[1] = self.k_states;
        self.tmp3 = cnp.PyArray_ZEROS(2, dim2, cnp.NPY_COMPLEX128, FORTRAN)
        self._tmp3 = &self.tmp3[0, 0]
...
cpdef _zcompute_coefficients_from_multivariate_pacf(cnp.complex128_t [::1,:] partial_autocorrelations,
...
cpdef _zconstrain_sv_less_than_one(cnp.complex128_t [::1,:] unconstrained, int order, int k_endog):
    """
    Transform arbitrary matrices to matrices with singular values less than
    one.
...
cdef int _zcopy_missing_submatrix(cnp.complex128_t * a, cnp.complex128_t * b, int * missing, int n):
    ...
    # Perform replacement
    for i in range(nobs):
        zcopy(&nobs, &a[i*n], &inc, &b[i*n], &inc)
...
cdef int _zcopy_missing_cols(cnp.complex128_t * a, cnp.complex128_t * b, int * missing, int n, int m):
    ...
    # Perform replacement
    for i in range(nobs):
        zcopy(&n, &a[i*n], &inc, &b[i*n], &inc)


cpdef int zcopy_missing_matrix(cnp.complex128_t [::1, :, :] A, cnp.complex128_t [::1, :, :] B, int [::1, :] missing, int missing_rows, int missing_cols, int diagonal) except *:
    cdef int n, m, T, t, A_T, A_t = 0, time_varying
...
cpdef int zcopy_index_matrix(cnp.complex128_t [::1, :, :] A, cnp.complex128_t [::1, :, :] B, int [::1, :] index, int index_rows, int index_cols, int diagonal) except *:
    cdef int n, m, T, t, A_T, A_t = 0, time_varying
