...
cdef np.complex64_t cinverse_univariate(
        cKalmanFilter kfilter, cStatespace model,
        np.complex64_t determinant) except *:
    """
    ...
    blas.ccopy(
        &model._k_endogstates, model._design, &inc,
        kfilter._tmp3, &inc)
...
cdef np.complex64_t cfactorize_cholesky(
        cKalmanFilter kfilter, cStatespace model,
        np.complex64_t determinant) except *:
    """
    ...
    if not kfilter.converged or not (kfilter.conserve_memory & MEMORY_NO_STD_FORECAST > 0):
        blas.ccopy(
            &kfilter.k_endog2, kfilter._forecast_error_cov, &inc,
            kfilter._forecast_error_fac, &inc)
        lapack.cpotrf(
            "U", &model._k_endog, kfilter._forecast_error_fac,
            &kfilter.k_endog, &info)
...
cdef np.complex64_t cfactorize_lu(
        cKalmanFilter kfilter, cStatespace model,
        np.complex64_t determinant) except *:
    """
...
cdef np.complex64_t cinverse_cholesky(
        cKalmanFilter kfilter, cStatespace model,
        np.complex64_t determinant) except *:
    """
    ...
    if not kfilter.converged or not (kfilter.conserve_memory & MEMORY_NO_STD_FORECAST > 0):
        ...
        if not (kfilter.conserve_memory & MEMORY_NO_STD_FORECAST > 0):
            blas.ccopy(
                &kfilter.k_endog, kfilter._forecast_error, &inc,
                kfilter._standardized_forecast_error, &inc)
            lapack.ctrtrs(
                "U", "T", "N", &kfilter.k_endog, &inc,
                kfilter._forecast_error_fac, &kfilter.k_endog,
    ...
    blas.cgemv(
        "N", &model._k_endog, &model._k_endog,
    ...
    blas.cgemm(
        "N", "N", &model._k_endog, &model._k_states, &model._k_endog,
        &alpha, kfilter._forecast_error_fac, &kfilter.k_endog,
    ...
    if not (kfilter.conserve_memory & MEMORY_NO_SMOOTHING > 0):
        ...
        blas.cgemm(
            "N", "N", &model._k_endog, &model._k_endog, &model._k_endog,
            &alpha, kfilter._forecast_error_fac, &kfilter.k_endog,
...
cdef np.complex64_t cinverse_lu(
        cKalmanFilter kfilter, cStatespace model,
        np.complex64_t determinant) except *:
    """
    inverse_cholesky(self, determinant)
    ...
    if not kfilter.converged:
        ...
        # Continue taking the inverse
        lapack.cgetri(
            &model._k_endog, kfilter._forecast_error_fac, &kfilter.k_endog,
...
cdef np.complex64_t csolve_cholesky(
    cKalmanFilter kfilter, cStatespace model,
    np.complex64_t determinant) except *:
    """
    ...
    blas.ccopy(
        ...
        kfilter._tmp2, &inc)
    lapack.cpotrs(
        "U", &model._k_endog, &inc, kfilter._forecast_error_fac,
        &kfilter.k_endog, kfilter._tmp2, &kfilter.k_endog, &info)

    # `tmp3` array used here, dimension $(p \times m)$  
    # $F_t \\#_3 = Z_t$
    if model._k_states == model.k_states and model._k_endog == model.k_endog:
        blas.ccopy(
            &kfilter.k_endogstates, model._design, &inc,
            kfilter._tmp3, &inc)
    else:
    ...
    lapack.cpotrs(
        "U", &model._k_endog, &model._k_states, kfilter._forecast_error_fac,
        &kfilter.k_endog, kfilter._tmp3, &kfilter.k_endog, &info)
    ...
    if not (kfilter.conserve_memory & MEMORY_NO_SMOOTHING > 0):
        ...
        if model._k_states == model.k_states and model._k_endog == model.k_endog:
            blas.ccopy(
                &kfilter.k_endog2, model._obs_cov, &inc,
                kfilter._tmp4, &inc)
        else:
            for i in range(model._k_endog): # columns
                for j in range(model._k_endog): # rows
        ...
        lapack.cpotrs(
            "U", &model._k_endog, &model._k_endog, kfilter._forecast_error_fac,
            &kfilter.k_endog, kfilter._tmp4, &kfilter.k_endog, &info)

    return determinant

cdef np.complex64_t csolve_lu(
        cKalmanFilter kfilter, cStatespace model,
        np.complex64_t determinant) except *:
    """
    inverse_cholesky(self, determinant)
    ...
    blas.ccopy(
        ...
        kfilter._tmp2, &inc)
    lapack.cgetrs(
        "N", &model._k_endog, &inc, kfilter._forecast_error_fac, &kfilter.k_endog,
    ...
    if model._k_states == model.k_states and model._k_endog == model.k_endog:
        ...
        for i in range(model._k_states):    # columns
            for j in range(model._k_endog): # rows
                kfilter._tmp3[j + i * kfilter.k_endog] = model._design[j + i * model._k_endog]
    lapack.cgetrs(
        "N", &model._k_endog, &model._k_states, kfilter._forecast_error_fac, &kfilter.k_endog,
        ...
        if model._k_states == model.k_states and model._k_endog == model.k_endog:
            ...
            for i in range(model._k_endog):     # columns
                for j in range(model._k_endog): # rows
                    kfilter._tmp4[j + i * kfilter.k_endog] = model._obs_cov[j + i * model._k_endog]
        lapack.cgetrs(
            "N", &model._k_endog, &model._k_endog, kfilter._forecast_error_fac, &kfilter.k_endog,
...
cdef np.float32_t sinverse_univariate(
        sKalmanFilter kfilter, sStatespace model,
        np.float32_t determinant) except *:
    """
    Factorize the forecast error covariance matrix using simple division
    in the case that the observations are univariate.
...
cdef np.float32_t sfactorize_cholesky(
        sKalmanFilter kfilter, sStatespace model,
        np.float32_t determinant) except *:
    """
    Factorize the forecast error covariance matrix using a Cholesky
...
cdef np.float32_t sfactorize_lu(
        sKalmanFilter kfilter, sStatespace model,
        np.float32_t determinant) except *:
    """
    Factorize the forecast error covariance matrix using an LU
...
cdef np.float32_t sinverse_cholesky(
        sKalmanFilter kfilter, sStatespace model,
        np.float32_t determinant) except *:
    """
    inverse_cholesky(self, determinant)
...
cdef np.float32_t ssolve_cholesky(
    sKalmanFilter kfilter, sStatespace model,
    np.float32_t determinant) except *:
    """
    solve_cholesky(self, determinant)
...
cdef np.complex128_t zinverse_univariate(
        zKalmanFilter kfilter, zStatespace model,
        np.complex128_t determinant) except *:
    """
    Factorize the forecast error covariance matrix using simple division
    in the case that the observations are univariate.
...
cdef np.complex128_t zfactorize_cholesky(
        zKalmanFilter kfilter, zStatespace model,
        np.complex128_t determinant) except *:
    """
    Factorize the forecast error covariance matrix using a Cholesky
...
cdef np.complex128_t zfactorize_lu(
        zKalmanFilter kfilter, zStatespace model,
        np.complex128_t determinant) except *:
    """
    Factorize the forecast error covariance matrix using an LU
...
cdef np.complex128_t zinverse_cholesky(
        zKalmanFilter kfilter, zStatespace model,
        np.complex128_t determinant) except *:
    """
    inverse_cholesky(self, determinant)
...
cdef np.complex128_t zinverse_lu(
        zKalmanFilter kfilter, zStatespace model,
        np.complex128_t determinant) except *:
    """
    inverse_cholesky(self, determinant)
...
cdef np.complex128_t zsolve_cholesky(
    zKalmanFilter kfilter, zStatespace model,
    np.complex128_t determinant) except *:
    """
    solve_cholesky(self, determinant)
...
cdef np.complex128_t zsolve_lu(
        zKalmanFilter kfilter, zStatespace model,
        np.complex128_t determinant) except *:
    """
    inverse_cholesky(self, determinant)
...
cdef np.float64_t dinverse_univariate(
        dKalmanFilter kfilter, dStatespace model,
        np.float64_t determinant) except *:
    """
    Factorize the forecast error covariance matrix using simple division
    in the case that the observations are univariate.
...
cdef np.float64_t dfactorize_cholesky(
        dKalmanFilter kfilter, dStatespace model,
        np.float64_t determinant) except *:
    """
    Factorize the forecast error covariance matrix using a Cholesky
...
cdef np.float64_t dfactorize_lu(
        dKalmanFilter kfilter, dStatespace model,
        np.float64_t determinant) except *:
    """
    Factorize the forecast error covariance matrix using an LU
...
cdef np.float64_t dinverse_cholesky(
        dKalmanFilter kfilter, dStatespace model,
        np.float64_t determinant) except *:
    """
    inverse_cholesky(self, determinant)
...
cdef np.float64_t dsolve_cholesky(
    dKalmanFilter kfilter, dStatespace model,
    np.float64_t determinant) except *:
    """
    solve_cholesky(self, determinant)
