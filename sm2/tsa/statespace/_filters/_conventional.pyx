...
cdef cnp.complex64_t cinverse_missing_conventional(cKalmanFilter kfilter, cStatespace model, cnp.complex64_t determinant)  except *:
...
cdef cnp.complex64_t cloglikelihood_missing_conventional(cKalmanFilter kfilter, cStatespace model, cnp.complex64_t determinant):
    return 0.0
...
cdef cnp.complex64_t cloglikelihood_conventional(cKalmanFilter kfilter, cStatespace model, cnp.complex64_t determinant):
    # Constants
    cdef:
...
cdef cnp.float32_t sinverse_missing_conventional(sKalmanFilter kfilter, sStatespace model, cnp.float32_t determinant)  except *:
    # Since the inverse of the forecast error covariance matrix is not
    # stored, we don't need to fill it (e.g. with NPY_NAN values). Instead,
    # just do a noop here and return a zero determinant ($|0|$).
    return 0.0

cdef cnp.float32_t sloglikelihood_missing_conventional(sKalmanFilter kfilter, sStatespace model, cnp.float32_t determinant):
    return 0.0
...
cdef cnp.float32_t sloglikelihood_conventional(sKalmanFilter kfilter, sStatespace model, cnp.float32_t determinant):
    # Constants
    cdef:
...
cdef cnp.complex128_t zinverse_missing_conventional(zKalmanFilter kfilter, zStatespace model, cnp.complex128_t determinant)  except *:
    # Since the inverse of the forecast error covariance matrix is not
    # stored, we don't need to fill it (e.g. with NPY_NAN values). Instead,
    # just do a noop here and return a zero determinant ($|0|$).
    return 0.0

cdef cnp.complex128_t zloglikelihood_missing_conventional(zKalmanFilter kfilter, zStatespace model, cnp.complex128_t determinant):
    return 0.0
...
cdef cnp.complex128_t zloglikelihood_conventional(zKalmanFilter kfilter, zStatespace model, cnp.complex128_t determinant):
    # Constants
    cdef:
...
cdef cnp.float64_t dinverse_missing_conventional(dKalmanFilter kfilter, dStatespace model, cnp.float64_t determinant)  except *:
    # Since the inverse of the forecast error covariance matrix is not
    # stored, we don't need to fill it (e.g. with NPY_NAN values). Instead,
    # just do a noop here and return a zero determinant ($|0|$).
    return 0.0

cdef cnp.float64_t dloglikelihood_missing_conventional(dKalmanFilter kfilter, dStatespace model, cnp.float64_t determinant):
    return 0.0
...
cdef cnp.float64_t dloglikelihood_conventional(dKalmanFilter kfilter, dStatespace model, cnp.float64_t determinant):
    # Constants
    cdef:
