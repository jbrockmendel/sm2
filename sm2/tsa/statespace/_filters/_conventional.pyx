...
cdef int zforecast_missing_conventional(
...
cdef int zupdating_missing_conventional(
        zKalmanFilter kfilter, zStatespace model):
    cdef:
        int inc = 1
...
cdef cnp.complex128_t zinverse_missing_conventional(
        zKalmanFilter kfilter, zStatespace model,
        cnp.complex128_t determinant)  except *:
    # Since the inverse of the forecast error covariance matrix is not
    # stored, we don't need to fill it (e.g. with NPY_NAN values). Instead,
    # just do a noop here and return a zero determinant ($|0|$).
    return 0.0

cdef cnp.complex128_t zloglikelihood_missing_conventional(
        zKalmanFilter kfilter, zStatespace model,
        cnp.complex128_t determinant):
    return 0.0
...
cdef int zforecast_conventional(
        zKalmanFilter kfilter, zStatespace model):
...
cdef int zupdating_conventional(
        zKalmanFilter kfilter, zStatespace model):
    # Constants
    cdef:
        int inc = 1
...
cdef int zprediction_conventional(
        zKalmanFilter kfilter, zStatespace model):
    cdef:
        int inc = 1
        cnp.complex128_t alpha = 1.0
...
cdef cnp.complex128_t zloglikelihood_conventional(
        zKalmanFilter kfilter, zStatespace model,
        cnp.complex128_t determinant):
    # Constants
    cdef:
...
cdef int cforecast_missing_conventional(
...
cdef int cupdating_missing_conventional(
        cKalmanFilter kfilter, cStatespace model):
    cdef:
        int inc = 1
...
cdef cnp.complex64_t cinverse_missing_conventional(
        cKalmanFilter kfilter, cStatespace model,
        cnp.complex64_t determinant)  except *:
    # Since the inverse of the forecast error covariance matrix is not
    # stored, we don't need to fill it (e.g. with NPY_NAN values). Instead,
    # just do a noop here and return a zero determinant ($|0|$).
    return 0.0

cdef cnp.complex64_t cloglikelihood_missing_conventional(
        cKalmanFilter kfilter, cStatespace model,
        cnp.complex64_t determinant):
    return 0.0
...
cdef int cforecast_conventional(
        cKalmanFilter kfilter, cStatespace model):
...
cdef int cupdating_conventional(
        cKalmanFilter kfilter, cStatespace model):
    # Constants
    cdef:
        int inc = 1
...
cdef int cprediction_conventional(
        cKalmanFilter kfilter, cStatespace model):
    cdef:
        int inc = 1
        cnp.complex64_t alpha = 1.0
...
cdef cnp.complex64_t cloglikelihood_conventional(
        cKalmanFilter kfilter, cStatespace model,
        cnp.complex64_t determinant):
    # Constants
    cdef:
