...
from sm2.genmod.families.links import identity
...
class MLEModel(tsbase.TimeSeriesModel):
    ...
    **kwargs
    ...
    --------
    MLEResults
    sm2.tsa.statespace.kalman_filter.KalmanFilter
    sm2.tsa.statespace.representation.Representation
    """
...
class MLEResults(tsbase.TimeSeriesModelResults):
    ...
    --------
    MLEModel
    sm2.tsa.statespace.kalman_filter.FilterResults
    sm2.tsa.statespace.representation.FrozenRepresentation
    """
    ...
    def test_normality(self, method):
        ...
        See Also
        --------
        sm2.stats.stattools.jarque_bera
        """
        if method is None:
            method = 'jarquebera'

        if method == 'jarquebera':
            from sm2.stats.stattools import jarque_bera
    ...
    def test_serial_correlation(self, method, lags=None):
        ...
        output : array
            ...
            `sm2.stats.diagnostic.acorr_ljungbox`) for the `i` th
            endogenous variable.
        ...
        See Also
        --------
        sm2.stats.diagnostic.acorr_ljungbox
        """
        if method is None:
            method = 'ljungbox'

        if method == 'ljungbox' or method == 'boxpierce':
            from sm2.stats.diagnostic import acorr_ljungbox
            d = self.loglikelihood_burn
            output = []
    ...
    def get_forecast(self, steps=1, **kwargs):
        ...
        **kwargs
        ...
        """
        if isinstance(steps, integer_types):
...
class PredictionResults(pred.PredictionResults):
    ...
    def __init__(self, model, prediction_results, row_labels=None):
        # TODO: Dont call it `model` if its really `results`
