...
def load_results_sm(dataset):
    ...
    for dt_s_tup in dt_s_list:
        ...
        results_per_deterministic_terms[dt_s_tup] = model.fit(maxlags=4,
                                                              trend=dt_s_tup[0],
                                                              method="ols")
        ...
        for dt in dt_s_list:
            ...
            for causing_ind in sublists(v_ind, 1, len(v_ind) - 1):
                ...
                assert_allclose(g_t_obt, g_t_des,
                                rtol, atol, False, err_msg_g_t)
                # check whether string sequences as args work in the same way:
                g_t_obt_str = granger_sm_str.test_statistic
                msg = err_msg_g_t + (" - sequences of integers and "
                                     "strings as arguments don't yield "
                                     "the same result!").upper()
                ...
                if len(causing_ind) == 1 or len(caused_ind) == 1:
                    ...
                    msg = err_msg_g_t + (" - list of int and int as "
                                         "argument don't yield "
                                         "the same result!").upper()
                ...
                assert_allclose(t_obt, t_des,
                                rtol, atol, False, err_msg_i_t)
                # check whether string sequences as args work in the same way:
                t_obt_str = inst_sm_str.test_statistic
                msg = err_msg_i_t + (" - sequences of integers and "
                                     "strings as arguments don't "
                                     "yield the same result!").upper()
                ...
                if len(causing_ind) == 1:
                    ...
                    msg = err_msg_i_t + (" - list of int and int as "
                                         "argument don't yield "
                                         "the same result!").upper()
                    assert_allclose(t_obt_single, t_obt,
                                    1e-07, 0, False, err_msg_i_t)
                ...
                assert_allclose(p_obt, p_des,
                                rtol, atol, False, err_msg_i_p)
                # check whether string sequences as args work in the same way:
                p_obt_str = inst_sm_str.pvalue
                msg = err_msg_i_p + (" - sequences of integers and "
                                     "strings as arguments don't yield "
                                     "the same result!").upper()
                ...
                if len(causing_ind) == 1:
                    ...
                    msg = (err_msg_i_p + " - list of int and int as ".upper() +
