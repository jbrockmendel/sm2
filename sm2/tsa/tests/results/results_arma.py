...
path = os.path.join(cur_dir, "yhat_exact_nc.csv")
yhat_mle = pd.read_csv(path).values

path = os.path.join(cur_dir, "yhat_css_nc.csv")
yhat_css = pd.read_csv(path).values

path = os.path.join(cur_dir, "yhat_exact_c.csv")
yhatc_mle = pd.read_csv(path).values

path = os.path.join(cur_dir, "yhat_css_c.csv")
yhatc_css = pd.read_csv(path).values

path = os.path.join(cur_dir, "resids_exact_nc.csv")
resids_mle = pd.read_csv(path).values

path = os.path.join(cur_dir, "resids_css_nc.csv")
resids_css = pd.read_csv(path).values

path = os.path.join(cur_dir, "resids_exact_c.csv")
residsc_mle = pd.read_csv(path).values

path = os.path.join(cur_dir, "resids_css_c.csv")
residsc_css = pd.read_csv(path).values

path = os.path.join(cur_dir, "results_arma_forecasts.csv")
...
class Y_arma11(object):
    def __init__(self, method="mle"):
        if method == "mle":
            ...
            # Note: bse, cov_params, and tvalues come from R;
            #       commented-out alternatives come
            #       from [not sure, but the same as all the other variables].
            #       See discussion in GH#4357.
...
class Y_arma14(object):
    def __init__(self, method="mle"):
        if method == "mle":
            ...
            self.bse = [0.06106330, 0.07381130, 0.07257705,
                        0.06857992, 0.07046048]
            self.cov_params = [
            ...
            self.tvalues = [12.643194, 3.847252, -11.501785,
                            2.301399, 6.126120]
...
class Y_arma41(object):
    def __init__(self, method="mle"):
        if method == "mle":
            ...
            self.arroots = [1.0209 - 0j, 0.2966 - 0.9835j,
                            0.2966 + 0.9835j, -1.4652 + 0.0000j]
...
class Y_arma22(object):
    def __init__(self, method="mle"):
        if method == "mle":
            self.params = [0.810898877154, -0.535753742985,
                           0.101765385197, -0.691891368356]
            self.aic = 756.286535543453
            self.bic = 773.893840132765
            self.arroots = [0.7568 - 1.1375j, 0.7568 + 1.1375j]
            self.maroots = [-1.1309, 1.2780]
            self.bse = [0.065073834100, 0.060522519771,
                        0.065569474599, 0.071275323591]
            self.cov_params = [
            ...
            self.params = [0.811172493623, -0.538952207139,
                           0.108020549805, -0.697398037845]
...
class Y_arma50(object):
    def __init__(self, method="mle"):
        if method == "mle":
            ...
            # NOTE: some results use x-12 arima because gretl
            # uses LS estimates for AR CSS
            ...
            self.bic = 697.8248
            self.arroots = [1.0755 + 0.0000j, 0.0075 - 1.2434j,
                            0.0075 + 1.2434j, -1.9686 + 0.0000j,
                            3.3994 + 0.0000j]
            self.maroots = None
...
class Y_arma11c(object):
    def __init__(self, method="mle"):
        if method == "mle":
            ...
            # Note: params, bse, cov_params, tvalues come from R;
            #       commented-out alternatives come
            #       from [not sure, but the same as all the other variables].
            #       See discussion in GH#4357.
...
class Y_arma14c(object):
    def __init__(self, method="mle"):
        if method == "mle":
            ...
            # had to change order in maroots?
            ...
            # Note: params, bse, cov_params, and tvalues come from R;
            #       commented-out alternatives come
            #       from [not sure, but the same as all the other variables].
            #       See discussion in GH#4357.
            ...
            # self.cov_params = [
            #   [0.28593, -0.059175, 0.053968, 0.046974,
            #    0.00085168, 0.0028000],
            #   [-0.059175, 0.012382, -0.011333, -0.0098375,
            #    -0.00012631, -0.00058518],
            #   [0.053968, -0.011333, 0.014362, 0.010298,
            #    -0.0028117, -0.00011132],
            #   [0.046974, -0.0098375, 0.010298, 0.012380,
            #    0.00031018, -0.0021617],
            #   [0.00085168, -0.00012631, -0.0028117, 0.00031018,
            #    0.0050069, 0.00079958],
            #   [.0028000, -0.00058518, -0.00011132, -0.0021617,
            #    0.00079958, 0.0038172]]
            ...
            self.maroots = [1.1306 - 0.7071j, 1.1306 + 0.7071j,
                            -1.3554 - 0.0896j, -1.3554 + 0.0896j]
            ...
            self.pvalues = [0.0050, 8.31e-10, 0.0991,
                            6.64e-12, 0.0528, 8.09e-07]
...
class Y_arma41c(object):
    def __init__(self, method="mle"):
        if method == "mle":
            ...
            self.arroots = [1.0077 + 0j, .3044 - .9793j,
                            .3044 + .9793j, -1.2466 + 0j]
            ...
            # Note: params, cov_params, bse, and tvalues come from R;
            #       commented-out alternatives come
            #       from [not sure, but the same as all the other variables].
            #       See discussion in GH#4357.
            ...
            # self.cov_params = [
            #   [0.0057834, 0.00052477, -0.00079965, 0.00061291,
            #    -0.00013618, -0.0018963],
            #   [0.00052477, 0.0046042, -0.0062505, 0.0053416,
            #    -0.0032941, -0.0047957],
            #   [-0.00079965, -0.0062505, 0.0096121, -0.0084500,
            #    0.0047967, 0.0064755],
            #   [0.00061291, 0.0053416, -0.0084500, 0.0082262,
            #    -0.0048029, -0.0057908],
            #   [-0.00013618, -0.0032941, 0.0047967, -0.0048029,
            #    0.0032869, 0.0035716],
            #   [-0.0018963, -0.0047957, 0.0064755, -0.0057908,
            #    0.0035716, 0.0099971]]
            ...
            self.arroots = [1.0141 + 0.0000j, 0.3036 - 0.9765j,
                            0.3036 + 0.9765j, -1.2455 + 0.0000j]
...
class Y_arma50c(object):
    def __init__(self, method="mle"):
        if method == "mle":
            ...
            self.pvalues = [1.50e-46, 9.06e-34, 6.53e-05,
                            0.0010, 0.0003, 0.0060]
            ...
            # self.cov_params = [
            #   [0.056132, -0.0028895, -0.0012291, -0.0031424,
            #     -0.0012502, -0.0028739],
            #   [-0.0028895, 0.0040416, -0.0029508, 0.0014229,
            #    -0.0012546, -0.00062818],
            #   [-0.0012291, -0.0029508, 0.0061163, -0.0041939,
            #    0.0025537, -0.0012585],
            #   [-0.0031424, 0.0014229, -0.0041939, 0.0062295,
            #    -0.0041928, 0.0014204],
            #   [-0.0012502, -0.0012546, 0.0025537, -0.0041928,
            #    0.0061127, -0.0029479],
            #   [-0.0028739, -0.00062818, -0.0012585, 0.0014204,
            #    -0.0029479, 0.0040445]]
            ...
            self.bic = 721.3522
            self.arroots = [-1.6539 + 0.0000j, 0.0091 - 1.2069j,
                            0.0091 + 1.2069j, 1.1508 + 0.0000j,
                            2.0892 + 0.0000j]
            self.maroots = None
...
class Y_arma02c(object):
    def __init__(self, method="mle"):
        if method == "mle":
            ...
            # Note: cov_params and tvalues come from R;
            #       commented-out alternatives come
            #       from [not sure, but the same as all the other variables].
            #       See discussion in GH#4357.
            # self.cov_params = [
