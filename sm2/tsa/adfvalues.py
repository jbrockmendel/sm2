...
# p = stats.norm.cdf(d_0 + d_1*log(abs(z)) + d_2*log(abs(z))**2 + d_3*log(abs(z))**3
...
# p = stats.norm.cdf(d_0 + d_1 * z + d_2*z**2 + d_3*z**3 + d_4*z**4)
...
def mackinnoncrit(N=1, regression ="c", nobs=np.inf):
    ...
    if nobs is np.inf:
        ...
        return np.polyval(eval("tau_"+reg+"_2010["+str(N-1)+",:,::-1].T"),1./nobs)
