    ...
    def _estimate_both(self, a, scale, mu, axis, est_mu, n):
        ...
        for _ in range(self.maxiter):
            ...
            if est_mu:
                ...
                nmu = mu.squeeze()
            nmu = np.expand_dims(nmu, axis)

            subset = np.less_equal(np.fabs((a - mu) / scale), self.c)
            card = subset.sum(axis)

            sqscale = (np.sum(subset * (a - nmu)**2, axis) /
                       (n * self.gamma - (a.shape[axis] - card) * self.c**2))
            nscale = np.sqrt(sqscale)
            nscale = np.expand_dims(nscale, axis)

            test1 = (np.fabs(scale - nscale) <= nscale * self.tol).all()
            test2 = (np.fabs(mu - nmu) <= nscale * self.tol).all()
...
class HuberScale(object):
    ...
    def __call__(self, df_resid, nobs, resid):
        h = df_resid / nobs * (
            self.d**2 + (1 - self.d**2) * stats.norm.cdf(self.d) -
            .5 -
