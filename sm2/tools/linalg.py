...
def lstsq(a, b, cond=None, overwrite_a=0, overwrite_b=0):
    ...
    Raises np.linalg.LinAlgError if computation does not converge
    """
    ...
    gelss, = scipy.linalg.lapack.get_lapack_funcs(('gelss',), (a1, b1))
    if n > m:
    ...
    if gelss.module_name[:7] == 'flapack':
        ...
        raise NotImplementedError('calling gelss from %s' %
                                  gelss.module_name)
    if info > 0:
        raise np.linalg.LinAlgError("SVD did not converge in "
                                    "Linear Least Squares")
    if info < 0:
...
def pinv2(a, cond=None, rcond=None):
    ...
    Raises np.linalg.LinAlgError if SVD computation does not converge

    Examples
    --------
    >>> a = np.random.randn(9, 6)
    >>> B = np.linalg.pinv2(a)
    >>> allclose(a, np.dot(a, np.dot(B, a)))
    True
    >>> allclose(B, np.dot(B, np.dot(a, B)))
    True
    """
    a = np.asarray_chkfinite(a)
    ...
    # TODO: XXX use lapack/blas routines for dot
