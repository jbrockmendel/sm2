...
from sm2.multivariate.tests.results.datamlw import data, princomp1, princomp2
...
class TestPCA(object):
    ...
    def test_warnings_and_errors(self):
        with warnings.catch_warnings(record=True) as w:
            PCA(self.x, ncomp=300)  # TODO: mark as smoke?
            assert_equal(len(w), 1)

        with warnings.catch_warnings(record=True) as w:
            rs = self.rs
            x = rs.standard_normal((200, 1)) * np.ones(200)
            PCA(x, method='eig')  # TODO: mark as smoke?
            assert_equal(len(w), 1)
    ...
    def test_gls_and_weights(self):
        ...
        pc_weights = PCA(x, ncomp=1, standardize=False,
                         demean=False, weights=weights)
